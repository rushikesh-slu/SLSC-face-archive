...
class Face():
    ...
    def __init__(self, filename, image, face_encoding,parent):
...
class Person():
    ...
    @classmethod
    def load(cls, pathname, face_encodings,file_info):
        ...
        for face_filename in os.listdir(pathname):
            ...
            if image.size == 0:
                continue
            if face_filename in file_info:
                parent_file = file_info[face_filename]
            else:
                parent_file = " "
            ...
            if face_encoding is None:
                ...
                face = Face(face_filename, image, face_encoding,parent_file)
...
class PersonDB():
    ...
    def load_db(self, dir_name):
        ...
        # read face_info
        pathname = os.path.join(dir_name, self.file_info)
        try:
            with open(pathname, "rb") as f:
                face_info = pickle.load(f)
                print(len(face_info), "face_info in", pathname)
        except:
            face_info = {}
        ...
        for entry in os.scandir(dir_name):
            if entry.is_dir(follow_symlinks=False):
                ...
                person = Person.load(pathname, face_encodings,face_info)
    ...
    def save_file_info(self, dir_name):
        face_encodings = {}
        for person in self.persons:
            for face in person.faces:
                face_encodings[face.filename] = face.parent_image
        for face in self.unknown.faces:
            face_encodings[face.filename] = face.parent_image
        pathname = os.path.join(dir_name, self.file_info)
        with open(pathname, "wb") as f:
    ...
    def save_db(self, dir_name):
        ...
        for person in self.persons:
            temp_lis = []
            for fac in person.faces:
                print(fac.filename)
                print(fac.parent_image)
                temp_lis.append(fac.parent_image)
            temp_set = set(temp_lis)
            temp_st = '|'.join(temp_set)
            di[person.name] = temp_st
            person.save_faces(dir_name)
        self.unknown.save_faces(dir_name)

        # self.save_montages(dir_name)
        self.save_encodings(dir_name)
        self.save_file_info(dir_name)
